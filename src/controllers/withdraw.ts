...
import {RESPONSE_CODE, WITHDRAW_APPLY_STATUS, WITHDRAW_MIN_AMOUNT} from "@src/infrastructure/utils/constants";
...
export default class Withdraw {
  ...
  async withdraw (ctx: IRouterContext) {
    ...
    if (user?.broadcaster) {
      const bills = await BillModel.find({target: uuid, createdAt: {$gt: new Date(user.freezeWithdrawTime), $lte: new Date(now)}}, {_id: 0, amount: 1}, {session});
