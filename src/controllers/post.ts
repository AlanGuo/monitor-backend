...
export default class PostsController {
  ...
  async getUserPosts(ctx: IRouterContext, next: any) {
    ...
    posts.forEach(item => {
      ...
      // 2. 是自己 可看
      // 3. 是粉丝 post收费 不可看
      console.log(item.payment.length > 0, isFan, item.from, uuid)
      item.payment = item.payment.length > 0 || item.from === uuid || (isFan ? item.price > 0 ? item.payment.length > 0 : false : false);
      item.media.forEach((media: { type: MEDIA_TYPE, fileName: string, [any: string]: any }) => {
        media.urls = getMediaUrl(media.type, media.fileName, item.payment, media.size);
        media.ready = true;
      })
